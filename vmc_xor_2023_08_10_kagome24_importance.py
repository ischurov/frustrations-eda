...
def main(task_id: int):
    ...
    writer = SummaryWriter(
        log_dir=(
            f"experiments/{datetime.now().strftime('%Y_%m_%d')}_vmc_xor_2023_08_10_kagome24/"
            f"{n_samples=}_{lr=}_{datetime.now().strftime('%H_%M_%S')}_importance"
        )
    )

    for outer_step in range(max_iter // inner_iter):
        ...
        for inner_step in range(inner_iter):
            step = outer_step * inner_iter + inner_step
