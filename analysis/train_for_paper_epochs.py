...
if mod_type == 'truncated':
	...
	data_test = df.drop(df_train.index).drop(df_val.index).sample(frac=test_eps, weights="prob")
	inputs_test, labels_test, probs_test = get_inputs_and_labels(data_test)

	data_original_test = df_original.drop(df_train.index).drop(df_val.index).sample(frac=test_eps, weights="prob")
	inputs_original_test, labels_original_test, probs_original_test = get_inputs_and_labels(data_original_test)
	...
	for epoch in range(epochs):  
			...
			print(f"Test set: accuracy: {100 * accuracy_test} %, sign overlap: {sign_overlap_test}")
			accuracy_or_test, sign_overlap_or_test = evaluate(net, inputs_original_test, labels_original_test, probs_original_test)
			print(f"Test set original: accuracy: {100 * accuracy_or_test} %, sign overlap: {sign_overlap_or_test}")
			truncated_acc.append(accuracy_test)
			truncated_acc_orig.append(accuracy_or_test)
###############################
...
if mod_type == 'flat':
	...
	for epoch in range(epochs):  
			...
			print(f"Test set: accuracy: {100 * accuracy_test} %, sign overlap: {sign_overlap_test}")
			accuracy_or_test, sign_overlap_or_test = evaluate(net, inputs_original_test, labels_original_test, probs_original_test)
			print(f"Test set: accuracy: {100 * accuracy_or_test} %, sign overlap: {sign_overlap_or_test}")
			truncated_acc.append(accuracy_test)
			truncated_acc_orig.append(accuracy_or_test)
...
if mod_type == 'nosign':
	...
	s1=series.truncate(keep_largest_n(N_COMPONENTS))
	t_coeffs=np.abs(np.sign(s1.coeffs))
	#np.random.shuffle(t_coeffs)
	s1.coeffs=t_coeffs
	prediction = s1.predict()
	ps=np.sign(prediction)
...
file = open('paper_final_epochs_200xors/'+lattice_type+'_j2='+str(j2_i)+'_'+mod_type+'_'+str(time_stamp)+'_modtest.pickle', 'wb')
pickle.dump(truncated_acc, file)
file.close()

file = open('paper_final_epochs_200xors/'+lattice_type+'_j2='+str(j2_i)+'_'+mod_type+'_'+str(time_stamp)+'_origtest.pickle', 'wb')
pickle.dump(truncated_acc_orig, file)
file.close()

x=range(0, len(truncated_acc))

plt.clf()
plt.plot(x,truncated_acc,color='red', linewidth=2, label='test on truncated')
plt.plot(x,truncated_acc_orig,color='blue', linewidth=2, label='test on original')
...
plt.ylim(0.3,1)
plt.ylabel('accuracy')
plt.title(str(N_COMPONENTS)+' components')
plt.legend(loc='lower right')

plt.savefig('paper_final_epochs_200xors/'+lattice_type+'_j2='+str(j2)+'_'+mod_type+'_terms_n='+str(N_COMPONENTS)+'.png')
plt.savefig('paper_final_epochs_200xors/'+lattice_type+'_j2='+str(j2)+'_'+mod_type+'_terms_n='+str(N_COMPONENTS)+'.eps')
